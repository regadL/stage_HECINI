elimcor_avecY<-function(X,Y,s=0.95) {
  ...
  while (stop==F) {
    ##regroupement des var pour lesquelles |corr|> s
    gplist<-list(NULL)
    ...
    if (any(gplisteff>1)) {
      ...
      gpsel=NULL
      ##on sélectionne dans chaque groupe la meilleure var par student
      for (i in 1:length(gplistfin)) {
        enrpval=rep(0,length(gplistfin[[i]]))
        for (j in 1:length(gplistfin[[i]])) {
	  #print (X[Y==1,possetap[gplistfin[[i]][j]]])
	  #print (X[Y==2,possetap[gplistfin[[i]][j]]])
          enrpval[j]=t.test(X[Y==1,possetap[gplistfin[[i]][j]]],X[Y==2,possetap[gplistfin[[i]][j]]])$p.val
        }
        ###si les variables sont égales, on en choisit une arbitrairement
        if (length(unique(enrpval))==1) selloc=min(gplistfin[[i]]) else selloc=gplistfin[[i]][which.min(enrpval)]
